...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Get push subscription + preferences
    const subDoc = await db.collection("push_subscriptions").doc("browser_main").get();
    if (!subDoc.exists) {
      return withCors({ skipped: true, reason: "no subscription" });
    }
    const subData = subDoc.data();
    const prefs = subData.prefs || {};
    if (!prefs.enabled) {
      return withCors({ skipped: true, reason: "notifications disabled" });
    }
    ...
    return withCors({ ok: true, sent, total: notifications.length });
  } catch (error) {
    console.error("check-notifications error:", error);
    return withCors({ error: error.message || "Check failed" }, 500);
  }
};
