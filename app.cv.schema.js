export const MASTER_CV_VERSION = "v11";

export const MASTER_CV_SCHEMA = Object.freeze({
...
export const CV_SECTION_DEFS = Object.freeze([
...
export const getCvSectionDefs = () => CV_SECTION_DEFS.map((section) => ({ ...section }));

export const getDefaultBaseCvSections = () => {
  const sections = {
...
export const normalizeTailoredCvSections = (sections = {}) => {
...
export const getResolvedCvSections = ({ baseSections = {}, tailoredSections = {} } = {}) => {
  ...
  CV_SECTION_DEFS.forEach((section) => {
    ...
    if (section.experienceId) {
      resolved.experience_overrides[section.experienceId] = [...resolved[section.key]];
    }
    if (section.competencyId) {
      resolved.competency_overrides[section.competencyId] = [...resolved[section.key]];
    }
  });
...
export const buildResolvedCvModel = ({ baseSections = {}, tailoredSections = {} } = {}) => {
  const resolvedSections = getResolvedCvSections({ baseSections, tailoredSections });
  const model = deepClone(MASTER_CV_SCHEMA);
  model.summary = resolvedSections.summary;
  model.key_achievements = [...resolvedSections.key_achievements];
  model.version = MASTER_CV_SCHEMA.version;
  model.experience = model.experience.map((entry) => {
    const key = EXPERIENCE_KEY_BY_ID[entry.id];
    if (key) {
      return { ...entry, bullets: [...resolvedSections[key]] };
    }
    return entry;
  });
  model.competencies = model.competencies.map((entry) => {
    const key = COMPETENCY_KEY_BY_ID[entry.id];
    if (key) {
      return { ...entry, items: [...resolvedSections[key]] };
    }
    return entry;
  });
  return model;
...
const evaluateBulletSection = ({ label, items = [], baseItems = [] } = {}) => {
  ...
  if (!stats.count) errors.push(`${label} is empty`);
  if (stats.count < Math.max(3, baseStats.count - 1)) warnings.push(`${label} is shorter than the master section`);
  if (stats.averageLength < baseStats.averageLength * 0.6) warnings.push(`${label} is materially thinner than the master bullets`);
  if (stats.genericPhraseCount > 1) warnings.push(`${label} contains generic bullet phrasing`);
  if (stats.weirdGlyphCount > 0) warnings.push(`${label} contains non-ATS-safe punctuation or glyphs`);
...
export const validateCvVariant = ({ baseSections = {}, tailoredSections = {} } = {}) => {
  const resolvedSections = getResolvedCvSections({ baseSections, tailoredSections });
...
export const finalizeTailoredCvSections = ({
  baseSections = {},
  tailoredSections = {},
  job = {},
  providerName = "",
...
export const buildMasterCvPromptText = () => {
  const lines = [];
